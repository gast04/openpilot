                       ...
                       )
{
  ...
  if (x_global >= 1 && x_local < 1) {
    ...
    globalColOffset = -1;
    cached[(y_local + 1) * localRowLen + x_local] = val_from_10(in, x_global-1, y_global, black_level, geometric_mean);
  } else if (x_global < RGB_WIDTH - 1 && x_local >= get_local_size(0) - 1) {
    localColOffset = x_local + 2;
    globalColOffset = 1;
    cached[localOffset + 1] = val_from_10(in, x_global+1, y_global, black_level, geometric_mean);
  }

  if (y_global >= 1 && y_local < 1) {
    cached[y_local * localRowLen + x_local + 1] = val_from_10(in, x_global, y_global-1, black_level, geometric_mean);
    if (localColOffset != -1) {
      cached[y_local * localRowLen + localColOffset] = val_from_10(in, x_global+globalColOffset, y_global-1, black_level, geometric_mean);
    }
  } else if (y_global < RGB_HEIGHT - 1 && y_local >= get_local_size(1) - 1) {
    cached[(y_local + 2) * localRowLen + x_local + 1] = val_from_10(in, x_global, y_global+1, black_level, geometric_mean);
    if (localColOffset != -1) {
      cached[(y_local + 2) * localRowLen + localColOffset] = val_from_10(in, x_global+globalColOffset, y_global+1, black_level, geometric_mean);
    }
  }
